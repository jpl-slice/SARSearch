...
class ASFClient:
    ...
    def __init__(self, config, logger):
        ...
        self.config = config
        self.logger = logger
        self.hyp3 = sdk.HyP3(username=config['hyp3_username'], password=config['hyp3_password'])
    ...
    def submit_jobs(self):
        ...
        for granule in granules:
            try:
                ...
                self.logger.error(f"Failed to submit job for granule {granule}: {str(e)}")

        self.hyp3.watch(batch)
        self.logger.info(f"All jobs submitted and being watched. Total jobs: {len(batch)}")
    ...
    def check_status(self):
        ...
        self.logger.info(f"Job Status - Succeeded: {len(succeeded)}, Failed: {len(failed)}, Running: {len(running)}")